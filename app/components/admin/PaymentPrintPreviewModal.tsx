...
interface PaymentPrintPreviewModalProps {
    ...
    singleRow?: PaymentRow;
}

export function PaymentPrintPreviewModal({
    paymentRows,
    isOpen,
    onClose,
    reportType,
    singleRow
}: PaymentPrintPreviewModalProps) {
    ...
    }, [isOpen, paymentRows, reportType, singleRow]);
    ...
    const generatePreview = async () => {
        ...
        try {
            ...
            if (reportType === 'single' && singleRow) {
                doc = await printSinglePayment(singleRow);
            } else {
                doc = await printAllPayments(paymentRows);
            }
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            if (reportType === 'single' && singleRow) {
                doc = await printSinglePayment(singleRow);
                doc.save(`payment-${singleRow.voucherNo}-${singleRow.vendorCode}.pdf`);
            } else {
                doc = await printAllPayments(paymentRows);
                    ...
                    <div className="flex items-center">
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs sm:text-sm text-gray-600 mt-0.5 sm:mt-1 truncate">
                                {reportType === 'single' && singleRow
                                    ? `Voucher: ${singleRow.voucherNo} - ${singleRow.vendorName}`
                                    : `Total Records: ${paymentRows.length}`
                                }
                            </p>
                        </div>
                    </div>
                    ...
                    {loading ? (
                        <div className="flex items-center justify-center h-full p-3 sm:p-4">
                            <div className="text-center">
                                ...
                                <p className="text-gray-500 text-xs sm:text-sm mt-1 sm:mt-2">Please wait while we prepare your payment report</p>
                            </div>
                        </div>
                    ) : error ? (
                        <div className="flex items-center justify-center h-full p-3 sm:p-4">
                        ...
                        <div className="flex items-center justify-center h-full p-3 sm:p-4">
                            <div className="text-center">
                                ...
                                <p className="text-gray-500 text-xs sm:text-sm mt-1 sm:mt-2">Unable to generate preview for this payment report</p>
                            </div>
                        </div>
                    )}
                </div>
                ...
                {/* <div className="p-2 sm:p-3 border-t border-gray-200 bg-white rounded-b-xl">
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
                        <div className="flex items-center p-2 sm:p-3 bg-gray-50 rounded-lg">
                            <DollarSign className="h-4 w-4 sm:h-5 sm:w-5 text-gray-500 mr-2 sm:mr-3 flex-shrink-0" />
                            <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center p-2 sm:p-3 bg-gray-50 rounded-lg">
                            <TrendingUp className="h-4 w-4 sm:h-5 sm:w-5 text-gray-500 mr-2 sm:mr-3 flex-shrink-0" />
                            <div className="min-w-0 flex-1">
                                <p className="text-xs text-gray-500 font-medium">Total Paid</p>
                                <p className="text-xs sm:text-sm font-semibold text-gray-900 truncate">
                                    ₹{paymentRows.reduce((sum, row) => sum + row.amountPaid, 0).toLocaleString('en-IN')}
                                </p>
                            </div>
                        </div>
                        <div className="flex items-center p-2 sm:p-3 bg-gray-50 rounded-lg">
                            <TrendingDown className="h-4 w-4 sm:h-5 sm:w-5 text-gray-500 mr-2 sm:mr-3 flex-shrink-0" />
                            <div className="min-w-0 flex-1">
                                <p className="text-xs text-gray-500 font-medium">Total Pending</p>
                                <p className="text-xs sm:text-sm font-semibold text-gray-900 truncate">
                                    ₹{paymentRows.reduce((sum, row) => sum + row.pendingAmount, 0).toLocaleString('en-IN')}
                                </p>
                            </div>
                        </div>
                        <div className="flex items-center p-2 sm:p-3 bg-gray-50 rounded-lg">
                            <Clock className="h-4 w-4 sm:h-5 sm:w-5 text-gray-500 mr-2 sm:mr-3 flex-shrink-0" />
                            <div className="min-w-0 flex-1">
                                <p className="text-xs text-gray-500 font-medium">Report Type</p>
                                <p className="text-xs sm:text-sm font-semibold text-gray-900 truncate capitalize">
                                    {reportType === 'single' ? 'Single Payment' : 'All Payments'}
                                </p>
                            </div>
                        </div>
                    </div>
